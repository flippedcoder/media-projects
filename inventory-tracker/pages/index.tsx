...
import { PlusCircleIcon, PencilAltIcon, TrashIcon } from '@heroicons/react/solid'
...
export default function Products({ products }) {
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-2">
        {products.length > 0 ? products.map((product: Product, index: number) => (
          <div key={product.id} className="flex gap-6 border-b-2 w-full">
            ...
            <PencilAltIcon className="h-6 w-6 text-indigo-500" onClick={() => {
              setIndex(index)
              setEditModal(true)
            }} />
            <TrashIcon className="h-6 w-6 text-red-400" onClick={() => deleteProduct(product.id)} />
          </div>
        )) :
      ...
      {editModal && <Modal product={products[index]} onCancel={() => setEditModal(false)} />}
    </div>
  )
}
